...
def is_valid_student_name(student_name: str) -> bool:
    """Allow letters, space and hyphen; disallow separators."""
...
def ask_student_name() -> str:
    while True:
        ...
        if is_valid_student_name(student_name):
            return student_name
        print("[client] Некорректное имя. Разрешены буквы, пробел и дефис.")
...
def main() -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        print("[client] Чат: сообщения; пустая строка или quit — выход.")

        msg = (
            f"student_name={student_name}; "
            f"student_group={student_group}; "
            f"student_number={student_number}; "
            "message=Hello from client (encrypted)."
        ).encode("utf-8")
        json_write(w, {"ciphertext_hex": xor_bytes(msg, key).hex()})
        ...
        while True:
            ...
            if not text or text.lower() == "quit":
                ...
                break
            json_write(
                w, {"ciphertext_hex": xor_bytes(text.encode("utf-8"), key).hex()}
            )
            reply = json_read(r)
