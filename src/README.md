...
**Протокол HTTP (HyperText Transfer Protocol)** — прикладной протокол передачи данных, лежащий в основе работы Всемирной паутины. HTTP работает по схеме «запрос—ответ» между клиентом (обычно браузером) и сервером.
